...
async function testNewSettings(): Promise<void> {
  ...
  const params = 'weeks' in result.params ? result.params.weeks[1] : result.params;

  console.log(`\nKey Parameters:`);
  console.log(`  Target Experts:     ${params.targetExperts}`);
  console.log(`  Hire Threshold:     ${(params.hireThreshold * 100).toFixed(0)}%`);
  console.log(`  Max Overtime:       ${params.maxOvertimeHours.toFixed(1)}h`);
  console.log(`  Overtime Threshold: ${(params.overtimeThreshold * 100).toFixed(0)}%`);
  console.log(`  MCE Allocation:     ${(params.mceCustomAllocation * 100).toFixed(1)}%`);
  console.log(`  Standard Price:     $${Math.round(225 * params.standardPriceMultiplier)}`);
